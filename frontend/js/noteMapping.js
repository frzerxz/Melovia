/**
 * Melovia - Note Mapping
 * Nota-frekans eşleştirme sistemi
 */

const NoteMapping = {
    // All notes with frequencies (Hz)
    notes: {
    ...
    getFrequency(noteName) {
        return this.notes[noteName] || null;
    },

    // Get Turkish name
    getTurkishName(noteName) {
        const note = noteName.replace(/[0-9]/g, '');
        const octave = noteName.match(/[0-9]/)?.[0] || '';
        return (this.turkishNames[note] || note) + octave;
    },

    // Get note from frequency (approximate)
    ...
    // Get note at semitone offset from base
    transpose(baseNote, semitones) {
        const note = baseNote.replace(/[0-9]/g, '');
        const octave = parseInt(baseNote.match(/[0-9]/)?.[0] || '4');
        ...
        return this.chromaticScale[noteIndex] + newOctave;
    }
};
